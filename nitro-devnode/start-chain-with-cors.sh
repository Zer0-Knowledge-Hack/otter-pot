...
# The CREATE2 factory deployed above creates contracts at
# addr = keccak256(0xff ++ factory ++ salt ++ keccak(code))[12:], with salt = 0.
deployer_code=$(cat ./nitro-devnode/stylus-deployer-bytecode.txt)
cast send --private-key $PRIVATE_KEY --rpc-url $RPC \
    $CREATE2_FACTORY "$SALT$deployer_code" >/dev/null

# Compute the deployed address (same formula the factory uses) with salt = 0.
code_hash=$(cast keccak $deployer_code)
raw="0xff$(echo $CREATE2_FACTORY | tr -d '0x')$(echo $SALT | tr -d '0x')$(echo $code_hash | tr -d '0x')"
deployer_address="0x$(cast keccak $raw | tr -d '0x' | tail -c 40)"
